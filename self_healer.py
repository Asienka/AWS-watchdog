...
def lambda_handler(event, context):
    ...
    try:
        response = client.get_alias(FunctionName=function_name, Name=alias_name)
        ...
        if current_version == "$LATEST" or not current_version.isdigit():
            print(
                f"Current version '{current_version}' is not a valid number or is $LATEST."
            )
            return {
                "status": "skipped",
                "message": "Already on $LATEST or non-numeric version",
            }
        if int(current_version) <= 1:
            print(
                f"Cannot rollback. Current version '{current_version}' is the earliest version."
            )
            return {
                "status": "skipped",
                "message": "Already at the earliest version, cannot rollback.",
            }
